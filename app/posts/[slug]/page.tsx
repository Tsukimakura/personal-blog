...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article className="article"><p className="muted">发布：{formatPostDate(post.publishedAt)} · 最近更新：{formatPostDate(post.updatedAt)} · {post.readingTime}</p><h1>{post.title}</h1><p className="lead">{post.description}</p><div className="tags">{post.tags.map(t => <Link className="tag" href={`/tags/${encodeURIComponent(t)}`} key={t}>#{t}</Link>)}</div><MDXRemote source={post.content} components={{ Collapsible }} options={{ mdxOptions: { remarkPlugins: [remarkMath], rehypePlugins: [rehypeKatex] } }} /></article>;
}
